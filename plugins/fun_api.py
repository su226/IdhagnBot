...
NBNHHSH_USAGE = "/能不能好好说话 <...缩写>\n只能输入字母和数字\n接口来自https://lab.magiconch.com/nbnhhsh/"
nbnhhsh = (command.CommandBuilder("fun_api.nbnhhsh", "能不能好好说话", "好好说话", "nbnhhsh")
  .brief("nbnhhsh?")
  .usage(NBNHHSH_USAGE)
  .build())
@nbnhhsh.handle()
...
couplet_parser.add_argument("-score", "-得分", metavar="分值", type=float, default=float("-inf"), help="下联的最小得分（默认：-∞）")
couplet = (command.CommandBuilder("fun_api.couplet", "对联")
  .brief("王斌给您对对联 -_-!")
...
async def handle_couplet(args: Namespace | ParserExit = ShellCommandArgs()):
  ...
  result = [(result, score) for result, score in zip(data["output"], data["score"]) if score > args.score]
  if not result:
...
ALIPAY_VOICE_USAGE = "/支付宝到帐 <金额>\n范围从0.01到999999999999.99，且只能有两位小数\n接口来自https://mm.cqu.cc/share/zhifubaodaozhang/"
alipay_voice = (command.CommandBuilder("fun_api.alipay_voice", "支付宝到帐", "支付宝", "zfb")
  .brief("支付宝到帐一亿元")
...
async def handle_alipay_voice(arg: Message = CommandArg()):
  ...
  await alipay_voice.finish(MessageSegment.record(f"https://mm.cqu.cc/share/zhifubaodaozhang/mp3/{value_str}.mp3"))

today = (command.CommandBuilder("fun_api.today", "今天", "today")
  .brief("查看历史上的今天")
