...
from util import color, command, helper, resources
...
async def handle_uptime(bot: Bot):
  ...
  im = Image.new("RGB", (image_width, image_height), color.split_rgb(CONFIG.background_color))
  draw = ImageDraw.Draw(im)
  draw.text((64, 64), INFO, color.split_rgb(CONFIG.secondary_color), font)
  ...
  if CONFIG.enable_account:
    ...
    draw.text(
      (left_padding, top_padding - line_height - 16), info["nickname"],
      color.split_rgb(CONFIG.primary_color), bold_font)
    draw.text(
      (left_padding + name_width, top_padding - line_height - 16), uid_str,
      color.split_rgb(CONFIG.secondary_color), font)
    draw.rectangle(
      (left_padding, top_padding - 6, left_padding + line_width, top_padding - 5),
      color.split_rgb(CONFIG.secondary_color))
  for i, (name, value) in enumerate(items):
    name_width = bold_font.getsize(name)[0]
    draw.text(
      (left_padding, top_padding + i * line_height), name,
      color.split_rgb(CONFIG.primary_color), bold_font)
    draw.text(
      (left_padding + name_width, top_padding + i * line_height), f": {value}",
      color.split_rgb(CONFIG.secondary_color), font)
  f = BytesIO()
  im.save(f, "PNG")
