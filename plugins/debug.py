...
from util import context, command, config_v2
...
api = (command.CommandBuilder("debug.api", "api")
  .level("super")
  .brief("调试机器人API")
  .usage('''\
/api <接口> [YAML数据]
...
say = (command.CommandBuilder("debug.say", "发送", "say", "s")
  .level("super")
  .brief("发送普通消息")
  .usage('''\
/发送 <消息>
...
raw = (command.CommandBuilder("debug.raw", "原始", "raw", "r")
  .level("super")
  .brief("发送含CQ码的消息")
  .usage('''\
/发送原始 <富文本消息>
...
execute = (command.CommandBuilder("debug.execute", "执行", "execute", "x")
  .level("super")
  .brief("在群内执行命令")
  .usage('''\
/执行 <命令名> [参数]
...
async def handle_execute(bot: Bot, event: MessageEvent, msg: Message = CommandArg()):
  ...
  if await redirect(bot, event, msg):
    await execute.send(f"已在 {context.GROUP_IDS[context.get_event_context(event)].name} 中执行命令")

reload = (command.CommandBuilder("debug.reload", "重载", "reload")
  .level("super")
  .brief("热重载支持的配置")
  .build())
@reload.handle()
