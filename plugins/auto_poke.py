...
auto_poke = nonebot.on_command("自动戳", context.in_context_rule(context.ANY_GROUP), {"autopoke"}, permission=context.SUPER)
...
/自动戳 <QQ号列表> - 设置自动戳
/自动戳 clear|清除 - 清除自动戳'''
auto_poke.__perm__ = context.SUPER
...
async def has_uid(event: Event) -> bool:
  return event.user_id in uids.get(context.get_context(event), [])
