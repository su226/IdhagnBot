...
parser.add_argument("--text", "-t", metavar="内容", default="", help=(
  "自定义内容，默认为空"
))
...
matcher = (
  command.CommandBuilder("meme_pic.teach", "讲课")
  .category("meme_pic")
  .brief("[动]")
  .shell(parser)
  .build()
...
async def handler(bot: Bot, event: MessageEvent, args: Namespace = ShellCommandArgs()) -> None:
  ...
  def make() -> MessageSegment:
    ...
    for raw in imutil.frames(target):
      frame = ImageOps.pad(raw.convert("RGBA"), (550, 395), imutil.scale_resample())
      im = Image.new("RGB", template.size, (0, 0, 0))
      im.paste(frame, (313, 60), frame)
