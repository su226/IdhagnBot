...
BOXES = [(1, 237), (826, 1), (832, 508), (160, 732)]
...
matcher = (
  command.CommandBuilder("meme_pic.smash", "砸")
  .category("meme_pic")
  .brief("[动]")
  .shell(parser)
  .build()
...
async def handler(bot: Bot, event: MessageEvent, args: Namespace = ShellCommandArgs()) -> None:
  ...
  def make() -> MessageSegment:
    ...
    for raw in imutil.frames(target):
      ...
      frame = ImageOps.pad(raw.convert("RGBA"), (826, 507), imutil.resample())
      frame = frame.rotate(18, imutil.resample(), True)
      im.paste(frame, (-133, -89), frame)
