...
from util import command, furbot_common, help, util
...
async def fetch_submit_image() -> MessageSegment:
  ...
  return util.local_image(SUBMIT_IMAGE_CACHE)
...
class Source:
  ...
  @staticmethod
  def keyword() -> str:
    return furbot_common.CONFIG().keyword

  @staticmethod
  def available() -> bool:
    return bool(furbot_common.CONFIG().token)
  ...
  async def handle(bot: Bot, event: Event, args: str) -> None:
    ...
    if args == "绒狸":
      ...
      return
    if not args:
      coro = furbot_common.get_random()
    else:
      try:
        id = int(args)
      except ValueError:
        coro = furbot_common.get_by_name(args)
      else:
        coro = furbot_common.get_by_id(id)
    try:
      pic = await coro
    except furbot_common.FurbotException as e:
      message = f"{HEADER}\n{e.message}"
      if e.code == 404:
...
picture = command.CommandBuilder("furbot.picture", "绒狸") \
  .category("furbot") \
  .brief("使用绒狸API的随机兽图") \
...
query = command.CommandBuilder("furbot.query", "绒狸查询") \
  .category("furbot") \
  .brief("查询毛毛的绒狸FID") \
  .usage("/绒狸查询 [名字]") \
  .help_condition(help_condition) \
...
async def handle_query(message: Message = CommandArg()):
  ...
  if not args:
    await query.finish("/绒狸查询 [名字]")
  if args.lower() in IDHAGN_NAMES:
    await query.finish(IDHAGN_EASTER_EGG)
  if args == "绒狸":
    await query.finish(EASTER_EGG_2)
  try:
    name, ids = await furbot_common.query_id(args)
  except furbot_common.FurbotException as e:
...
submit = command.CommandBuilder("furbot.submit", "绒狸投稿") \
  .category("furbot") \
  .brief("向绒狸投稿毛毛") \
  .help_condition(help_condition) \
  .rule(rule) \
...
daily = command.CommandBuilder("furbot.daily", "每日鉴毛") \
  .category("furbot") \
  .brief("绒狸每日鉴毛") \
  .usage('''\
/每日鉴毛 - 随机一期每日鉴毛
...
async def handle_daily(bot: Bot, event: Event, message: Message = CommandArg()):
  ...
  if args == "绒狸":
    ...
    try:
      ...
      coro = furbot_common.get_daily_by_name(args)
    else:
      coro = furbot_common.get_daily_by_id(id)
  try:
    pic = await coro
  except furbot_common.FurbotException as e:
...
@furbot_common.CONFIG.onload()
def onload(prev: furbot_common.Config | None, curr: furbot_common.Config):
  usage = USAGE_BASE
  if curr.keyword:
    usage += "\n" + USAGE_KEYWORD.format(curr.keyword)
  item = help.CommandItem.find("绒狸")
  item.raw_usage = usage
  global USAGE
  USAGE = usage

furbot_common.CONFIG.reloadable = "eager"
furbot_common.CONFIG.load()
