...
def render_vertical_text(content: str, font: str, size: int, **kw) -> Image.Image:
  ...
  for line in content.splitlines():
    ...
    while match := CHINESE_RE.search(line, pos):
      im = text.render(line[pos:match.start()], font, size, **kw)
      parts.append(im)
      width += im.width
      height = max(height, im.height)

      layout = text.layout(match[0], font, size)
      desc = layout.get_font_description()
      desc.set_gravity(Pango.Gravity.EAST)
      layout.set_font_description(desc)
      im = text.render(layout, **kw)
      parts.append(im)
      width += im.width
      height = max(height, im.height)
    ...
    im = text.render(line[pos:], font, size, **kw)
    parts.append(im)
    width += im.width
    height = max(height, im.height)
...
matcher = command.CommandBuilder("petpet_v2.book", "看书") \
  .category("petpet_v2") \
  .shell(parser) \
  .auto_reject() \
  .build()
...
async def handler(
  ...
  def make() -> MessageSegment:
    ...
    target = ImageOps.fit(target, OLD_SIZE, util.scale_resample)
    ...
    if args.text:
      ...
      text_im = util.contain_down(text_im, 240, 780)
      text_im = text_im.rotate(3, util.resample, True)
      util.paste(im, text_im, (990, 1890), anchor="mm")
