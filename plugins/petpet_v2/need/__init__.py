...
parser = ArgumentParser("/需要", add_help=False)
parser.add_argument("target", nargs="?", default="", metavar="目标", help="可使用@、QQ号、昵称、群名片或图片链接")
matcher = nonebot.on_shell_command("需要", parser=parser)
matcher.__cmd__ = ["需要"]
...
async def handler(bot: Bot, event: Event, args: Namespace | ParserExit = ShellCommandArgs()):
  ...
  avatar = avatar.resize((115, 115), Image.ANTIALIAS)
  im.paste(avatar, (327, 232), avatar)
