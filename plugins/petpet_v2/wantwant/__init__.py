...
from util import command, helper, context, text
...
matcher = (command.CommandBuilder("petpet_v2.wantwant", "旺仔", "旺旺")
  .category("petpet_v2")
  .brief("李子明的最爱")
  .shell(parser)
  .build())
...
async def handler(bot: Bot, event: MessageEvent, args: Namespace | ParserExit = ShellCommandArgs()) -> None:
  ...
  if text_im.width > 355:
    text_im = text_im.resize((355, text_im.height), Image.ANTIALIAS)
