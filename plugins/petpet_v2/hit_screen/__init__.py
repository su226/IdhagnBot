...
matcher = command.CommandBuilder("petpet_v2.hit_screen", "打穿", "打穿屏幕") \
  .brief("[动]") \
  .category("petpet_v2") \
  .shell(parser) \
  .auto_reject() \
...
async def handler(
  ...
  def make() -> MessageSegment:
    ...
    for i, raw in zip(range(6, 22), util.sample_frames(target, FRAMETIME)):
      template = Image.open(DIR / f"{i}.png").convert("RGBA")
      pos, size, data = TRANSFORMS[i - 6]
      fg = ImageOps.pad(raw.convert("RGBA"), (140, 120), util.scale_resample)
      fg = fg.transform(size, Image.Transform.PERSPECTIVE, data, util.resample)
