...
matcher = command.CommandBuilder("petpet_v2.sign", "举牌") \
  .category("petpet_v2") \
  .shell(parser) \
  .auto_reject() \
  .build()
...
async def handler(
  ...
  def make() -> MessageSegment:
    nonlocal target
    target = ImageOps.pad(target, OLD_SIZE, util.scale_resample)
    frames: list[Image.Image] = []
    for i, transform in enumerate(TRANSFORMS):
      pos, size, data = transform
      template = Image.open(DIR / f"{i}.png").convert("RGBA")
      im = Image.new("RGB", template.size, (249, 249, 249))
      fg = target.transform(size, Image.Transform.PERSPECTIVE, data, util.resample)
      im.paste(fg, pos, fg)
