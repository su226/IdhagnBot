...
def range_float(min_value: float, max_value: float) -> Callable[[str], float]:
...
def apply_watermark(watermarks: list[str], rand: np.random.Generator, im: Image.Image) -> None:
  ...
  text_im = text.render("@" + name, "sans", size, color=(255, 255, 255))
  blur = int(0.15 * size)
  ...
  draw.bitmap((x - blur, y - blur + shadow_offset), shadow_im, (0, 0, 0))
...
parser.add_argument(
  "--repeat", "-r", type=range_int(1, 20), default=12,
  help="做旧次数，为 [1, 20] 之间的整数，默认: 12")
parser.add_argument(
  "--blur", "-b", type=range_float(0, 100), default=0,
  help="模糊强度，为 [0, 100] 之间的小数，默认: 0")
parser.add_argument(
  "--quality", "-q", type=range_int(0, 101), default=60,
  help="JPEG压缩质量，为[0, 101] 之间的整数，101为不压缩，默认: 60")
parser.add_argument(
  "--noise", "-n", type=range_int(0, 127), default=0,
  help="噪点强度，为[0, 127] 之间的整数，默认: 0")
parser.add_argument(
  "--no-member-name", "-m", action="store_false", dest="member_name",
  help="不使用群员的名字作为水印")
parser.add_argument(
  "--watermark", "-a", action="append", default=[],
  help="自定义水印，可以指定多次，你可能需要配合 --no-member-name 使用")
parser.add_argument(
  "--offset", "-o", type=range_float(0, 100), default=0.5,
  help="随机偏移，默认: 0.5")
...
matcher = (
  command.CommandBuilder(
    "petpet_v2.patina", "电子包浆", "赛博包浆", "电子做旧", "赛博做旧", "包浆", "做旧")
  .category("petpet_v2")
  .brief("[动]")
  .shell(parser)
  .build())
