...
parser.add_argument("target",
  nargs="?", default="", metavar="目标",
  help="可使用@、QQ号、昵称、群名片或图片链接")
group = parser.add_mutually_exclusive_group()
group.add_argument("--webp", "-w",
  action="store_const", dest="format", const="webp", default="gif",
  help="使用WebP而非GIF格式")
group.add_argument("--png", "--apng", "-p",
  action="store_const", dest="format", const="png",
  help="使用APNG而非GIF格式")
matcher = command.CommandBuilder("petpet_v2.shock", "震惊") \
  .category("petpet_v2") \
  .shell(parser) \
  .build()
@matcher.handle()
async def handler(
  ...
  for _ in range(30):
    frames.append(
      motion_blur(avatar, random.randint(-90, 90), random.randint(0, 50))
